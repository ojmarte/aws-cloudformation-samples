...
if __name__ == "__main__":
    ...
    args = getResolvedOptions(sys.argv, ["pOrg", "pDomain", "pEnvironment"])
    org = args["pOrg"]
    domain = args["pDomain"]
    environment = args["pEnvironment"]

    landing_s3 = f"{org}-{domain}-{environment}-landing-bucket"
    processed_s3 = f"{org}-{domain}-{environment}-processed-bucket"
